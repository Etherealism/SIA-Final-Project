...
def done_task(db: Session = Depends(get_db), current_user: User = Depends(get_token)):
    ...
    tasks = db.query(Task).filter(
        current_user['author_id'] == Task.author_id,Task.status==1,Task.task_is_complete==1).all()
    return {'tasks': [
        {
            'tasks_id': task.task_id,
            'author_id': task.author_id,
...
@router.get('/{id}')
def specific_task(id:str,db: Session = Depends(get_db), current_user: User = Depends(get_token)):
